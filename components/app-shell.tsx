...
export async function AppShell({
  ...
  /** Optional override; falls back to "Vehicle Booking". */
  title?: string;
  roleBadge: string;
  ...
  const tint = ROLE_TINT[roleBadge] ?? "bg-muted text-muted-foreground ring-border";
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 flex h-14 items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            <Link href="/" className="flex items-center gap-2 font-semibold tracking-tight">
              ...
              <span className="hidden sm:inline">Vehicle Booking</span>
            </Link>
            <span
              className={`hidden md:inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ring-1 ring-inset ${tint}`}
            >
              {roleBadge}
            </span>
          </div>
