...
import { Wand2, GripVertical, AlertTriangle, Plus, Truck } from "lucide-react";
...
export function SchedulerBoard({
  ...
  // Outside-vehicle add form: hidden until asked for, like the rounds board's.
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <div className="overflow-x-auto rounded-xl border">
          <div className="min-w-[64rem]">
            {/* Same header and same add form as the rounds board's panel
                (components/admin/adhoc-rows-panel.tsx): one icon + title, one
                outline button that reveals the form. They were two different
                shapes for the same job, and the timeline's version could not
                capture the vendor's driver/phone at all. */}
            <div className="flex flex-wrap items-center justify-between gap-2 border-b bg-muted/30 px-4 py-2.5">
              ...
              {!addingRow && (
                <Button type="button" size="sm" variant="outline" onClick={() => setAddingRow(true)}>
            ...
            {addingRow && (
              <form
                ...
                className="flex flex-wrap items-end gap-2 border-b bg-muted/30 px-4 py-3"
              >
                <Input
                  autoFocus
                  name="label"
